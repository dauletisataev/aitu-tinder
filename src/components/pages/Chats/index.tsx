...
export const ChatsPage: React.FC = () => {
  ...
  useEffect(() => {
    const newMatches = matches.filter((match) => chats.map((chat) => chat.user.id).includes(match.id))
    setMatches((prevMatches) => prevMatches.length === newMatches.length ? prevMatches : newMatches);
  }, [matches, chats])
        ...
        <div className="mt-4">
          ...
          <div className="mt-3 overflow-x-scroll flex ">
            {matches.map((match, index) =>
              <img onClick={() => onMatchClick(match.id)} key={index} src={match.avatar_url || mockAvatar} className="ml-2 rounded-full w-14 h-14 mr-1" />
            )}
          </div>
        </div>
      )}
